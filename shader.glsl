...
{ 
  ...
  //float d = length(p) - (sin(fGlobalTime*0.25)+0.8); // animated radius
  
  // offset distance with pyroclastic noise
  //p = normalize(p) * _SphereRadius; // project noise point to sphere surface
  displace = fbm(p*_NoiseFreq + _NoiseAnim*fGlobalTime);
...
void main(void)
{
    ...
    float rotx = 0.0; //fGlobalfGlobalTime; // 0.0; //mouse.y*4.0;
